...
const __filenameVal = isESM ? fileURLToPath(import.meta.url) : (typeof __filename !== 'undefined' ? __filename : '');
const __dirnameVal = isESM ? path.dirname(__filenameVal) : (typeof __dirname !== 'undefined' ? __dirname : '');
...
async function startServer() {
  ...
  app.post('/api/media/upload', upload.single('mediaFile'), (req, res) => {
    try {
      ...
      const { title, description, type, passcode } = req.body;
      const requestPasscode = passcode || req.headers['x-passcode'];
      if (requestPasscode !== '2g9Hwdm-jzb') {
        if (req.file) {
          fs.unlinkSync(req.file.path);
        }
        return res.status(403).json({ error: 'Unauthorized: Incorrect passcode' });
      }
      ...
      const db = readDb();
      const originalNameUtf8 = req.file ? Buffer.from(req.file.originalname, 'latin1').toString('utf8') : '';
      const newMedia = {
        id: 'upload-' + Date.now(),
        title,
        ...
        fileName: originalNameUtf8 || req.file.originalname,
  ...
  app.post('/api/media/url', (req, res) => {
    try {
      const { title, description, type, url, passcode } = req.body;
      const requestPasscode = passcode || req.headers['x-passcode'];
      if (requestPasscode !== '2g9Hwdm-jzb') {
        return res.status(403).json({ error: 'Unauthorized: Incorrect passcode' });
      }
  ...
  app.post('/api/wishes', (req, res) => {
    try {
      const { sender, message, passcode } = req.body;
      const requestPasscode = passcode || req.headers['x-passcode'];
      if (requestPasscode !== '2g9Hwdm-jzb') {
        return res.status(403).json({ error: 'Unauthorized: Incorrect passcode' });
      }
  ...
  app.delete('/api/media/:id', (req, res) => {
    try {
      ...
      const requestPasscode = req.headers['x-passcode'] || req.query.passcode;
      if (requestPasscode !== '2g9Hwdm-jzb') {
        return res.status(403).json({ error: 'Unauthorized: Incorrect passcode' });
      }
